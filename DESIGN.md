...
> **Do Voltura ficou o desenho, não a pele.** O dono do produto testou a pele do pacote — campo citrino, lima ácido, Space Grotesk — e recusou: a identidade do GranaFy continua sendo o verde `#12B85C` sobre branco, com a fonte Geist. O que foi adotado é a **composição**: a casca de ponta a ponta, a barra lateral colada à esquerda, a faixa do topo, a grade de cartões, o bloco de KPI, as listas e o espaçamento. Ao mexer em UI, siga a composição descrita acima e a paleta desta seção — não a paleta nem a tipografia do topo deste arquivo.

- **Onde está**: `client/src/styles/voltura.css`. É o `system.css` do pacote com escopo em `.voltura` (os tokens saem de `:root`, e `body`, `::selection` e `:focus-visible` ficam presos ao escopo) e com a paleta e a fonte trocadas pelas do produto. O pacote original (licença, `metadata.json`, `html/preview.html`, `html/cover.html`, `images/`) está em `design/voltura/`, só para consulta.
- **Onde é usado**: em todas as telas do painel, nos dois temas — Visão geral, Fluxo de caixa, A pagar e receber, Pagas e recebidas, Lançamentos, Conciliação, Relatórios (o `RelatorioShell` cobre o hub e os sete), DRE, Balanço, Contas e categorias e Configurações. Ficam de fora a landing e as telas de entrada (login, cadastro, convite, senha, termos), que não têm a casca do painel.
- **Cor e fonte**: `.voltura` carrega a paleta clara do produto (canvas `#EFF4F1`, cartão branco, texto `#0B1F14`, verde `#12B85C`, verde de texto `#0A7A42`) e `.dark .voltura` a escura, com os mesmos valores da tabela de `client/src/index.css`. `--v-font-display`, `--v-font-body` e `--v-font-mono` apontam todos para Geist. Os nomes dos tokens continuam sendo os do pacote (`--v-acid`, `--v-slate`, `--v-moss`…): só o valor mudou. Cor nova entra como token nos dois blocos, nunca como valor solto na marcação.
- **O cartão de destaque** (`.v-card--focal`) é a superfície aurora do produto: verde-escuro com texto branco, o mesmo cartão de "Caixa disponível" de antes. As tintas de dentro dele saem de `--v-on-acid-*`.
- **Visão geral**: `client/src/pages/visaogeral/VisaoGeralPainel.tsx` é o desenho, remarcado com as classes `v-*` do pacote; os dados vêm de `useVisaoGeral`, no mesmo diretório; `Home.tsx` só junta os dois e mantém o modal de lançamento fora do escopo do painel.
- **Como vestir uma tela nova**: trocar a raiz por `<main className="voltura vg-pagina">`, tirar o vão do embrulho (`flex w-full`) e pôr `vg-casca vg-conteudo` na coluna de conteúdo. O resto vem de graça: dentro de `.vg-casca` existe uma tabela que traduz as classes de cor do app (`bg-white`, `text-[#4C6355]`, `bg-[#12B85C]`…) para os tokens, espelhando a tabela do tema escuro em `client/src/index.css`. Como os tokens são as cores do produto, no tema claro a tradução é praticamente identidade — ela existe para o tema escuro e para a tela não escapar quando um token mudar.
- **O que a casca muda na tela traduzida**: só o `<header>`, que vira a faixa com filete do painel. Tipo, raio e sombra continuam os da tela — foi o que o dono do produto pediu ao dizer "mesma fonte, mesmas cores, só o layout".
- **Como aplicar num desenho novo**: classes `v-*` para componentes (cartão, cartão focal, KPI, botões, chips, pill-tabs, lista, barra de distribuição) e classes `vg-*` (no fim do mesmo arquivo) para a composição da página. Tailwind serve para layout (`flex`, `grid`, `gap`), nunca para cor dentro do escopo.
- **Botões** usam o raio de 12px do produto, não a pílula do pacote, porque o mesmo botão existe nas telas de fora. A pílula fica em chip, barra de distribuição, pontos e avatares.
- **Ícones**: dentro do painel da Visão geral são Tabler inline (`TABLER` em `VisaoGeralPainel.tsx`); no resto do app seguem os Iconly de sempre.
- **Modal "Novo lançamento"**: o componente é compartilhado e mantém a marcação dele; o que o veste é um embrulho `.voltura` em volta e um bloco preso ao `.drawer-enter`, no fim do `voltura.css`.
- **Conflitos conhecidos**: o tema escuro do app repinta as classes de cor com `!important`; as sobreposições dos componentes compartilhados (barra lateral, menu do perfil, modal) precisam de `!important` e de especificidade maior que `.dark .bg-white` para valer. Componentes compartilhados dentro do escopo (`HideValuesButton`, `ProfileMenu`, `AppSidebar`, `VisaoGeralVazia`, `VisaoGeralSkeleton`) mantêm a marcação do app e recebem só cor e forma.
