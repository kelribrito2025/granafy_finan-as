...
- **Onde é usado**: na Visão geral, no Fluxo de caixa, em A pagar e receber, em Lançamentos e em Contas e categorias — nos dois temas. As telas que faltam (Conciliação, Relatórios, DRE, Balanço, Pagas e recebidas, Configurações) continuam com o visual antigo do app. `client/src/pages/visaogeral/VisaoGeralPainel.tsx` é o desenho, e é um só: importa o CSS, envolve a página em `.voltura` e recebe os dados de `useVisaoGeral`, no mesmo diretório. `Home.tsx` só junta os dois e mantém o modal de lançamento fora do escopo.
