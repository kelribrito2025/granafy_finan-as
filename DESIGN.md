...
- **Onde é usado**: somente na Visão geral, e só no tema escuro. `client/src/pages/Home.tsx` escolhe pelo tema já resolvido: claro renderiza `VisaoGeralClara` (o painel de sempre) e escuro renderiza `VisaoGeralEscura`, que importa o CSS e envolve a página em `.voltura`. Os dados dos dois desenhos vêm do mesmo `useVisaoGeral`, em `client/src/pages/visaogeral/`. As demais telas, a barra lateral (`AppSidebar`), o menu de perfil e os modais continuam com o visual anterior do app (Geist, Tailwind 4, verde `#12B85C`). Estender a outra tela é decisão do dono do produto.
- **Como aplicar**: classes `v-*` do sistema para componentes (cartão, cartão focal, KPI, botões, chips, pill-tabs, lista, barra de distribuição) e classes `vg-*` (no fim do mesmo arquivo) só para a composição da página. Tailwind pode ser usado para layout (`flex`, `grid`, `gap`), nunca para cor ou tipografia dentro do escopo.
