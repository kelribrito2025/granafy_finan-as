...
- **Conflitos conhecidos**: o sistema é dark-first e a barra lateral do app é clara — na Visão geral elas convivem lado a lado sobre o campo citrino. Componentes compartilhados que aparecem dentro do escopo (`HideValuesButton`, `ProfileMenu`, `VisaoGeralVazia`, `VisaoGeralSkeleton`, `TransactionModal`) mantêm o desenho do app; o olhinho ganhou uma sobreposição (`.vg-olho`) para caber no cartão focal.
