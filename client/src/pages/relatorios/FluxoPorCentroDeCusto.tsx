...
import { EstadoVazioRelatorio, RelatorioShell, dinheiro, mil, numero, type Janela } from "@/components/relatorios/RelatorioShell";
import { useSomenteLeitura } from "@/hooks/useSomenteLeitura";
import { CENTROS_MOCK, MESES_CURTOS_MOCK, PERIODO_MOCK, usarVazio } from "@/lib/relatoriosMock";
...
export default function FluxoPorCentroDeCusto() {
  ...
  const centros = CENTROS_MOCK.map(c => ({ ...c, saldoFinal: c.saldoInicial + c.entradas - c.saidas }));
  ...
  const totaisDoMes = MESES_CURTOS_MOCK.map((_, i) => centros.reduce((s, c) => s + (c.saidasPorMes[i] ?? 0), 0));
  const teto = tetoDoEixo(Math.max(...totaisDoMes));
  ...
  return (
    <RelatorioShell
      ...
      subtitulo={vazio ? "nenhum centro de custo cadastrado" : `${centros.length} centros de custo ativos · ${PERIODO_MOCK.de} a ${PERIODO_MOCK.ate}`}
      vazio={vazio}
      ...
      rodape={!vazio && emQueda.length > 0
        ...
        : "Só lançamentos pagos com centro de custo informado. Os demais ficam fora deste relatório."}
    >
        ...
        <>
          <section className="flex flex-col gap-[22px] rounded-[20px] bg-white p-[26px] ring-1 ring-[#E1E8E3]">
            <div className="flex flex-wrap items-baseline gap-3">
              ...
              <span className="ml-auto flex flex-wrap justify-end gap-3.5 text-[12px] text-[#4C6355]">
                {centros.map(c => <span key={c.id} className="flex items-center gap-1.5 whitespace-nowrap"><span className="h-3 w-3 rounded" style={{ background: c.cor }} />{c.nome}</span>)}
              </span>
            </div>
            <div className="flex gap-4">
              ...
              <div className="flex min-w-0 flex-1 items-start gap-2.5 overflow-x-auto border-l border-[#E3EBE6]">
                {MESES_CURTOS_MOCK.map((mes, i) => (
                  <div key={mes} className="flex min-w-[64px] flex-1 flex-col items-center gap-2.5">
                    <div className="flex h-[200px] w-[52px] flex-col justify-end overflow-hidden rounded-t-[7px]">
                      {[...centros].reverse().map(c => (
                        <span key={c.id} title={`${c.nome} · ${mil(c.saidasPorMes[i] ?? 0)}`} style={{ height: `${((c.saidasPorMes[i] ?? 0) / teto) * 200}px`, background: c.cor }} />
                      ))}
                    </div>
                    <span className="text-[12.5px] text-[#4C6355]">{mes}</span>
          ...
          <section className="rounded-[20px] bg-white p-[26px] ring-1 ring-[#E1E8E3]">
            <div className="flex flex-wrap items-baseline gap-3 pb-3">
              ...
              <span className="text-[12.5px] text-[#4C6355]">rateios já aplicados conforme a regra de cada centro</span>
            </div>
            <div className="overflow-x-auto">
              <div className="min-w-[860px]">
                ...
                <div className="grid grid-cols-[1.6fr_140px_140px_140px_150px_120px] items-center gap-3 px-1 pb-1 pt-4">
                  ...
                  <span className="text-right text-[16px] font-bold text-[#0A7A42]">{dinheiro(totalFinal)}</span>
