...
import { CarregandoRelatorio, ErroDoRelatorio, EstadoVazioRelatorio, RelatorioShell, dinheiro, numero } from "@/components/relatorios/RelatorioShell";
import { useSomenteLeitura } from "@/hooks/useSomenteLeitura";
import { useRelatorioDeFluxo } from "@/lib/relatorios";
...
/** O peso da conta no caixa: só faz sentido com total positivo e conta não negativa. */
function peso(saldo: number, total: number) {
  if (total <= 0 || saldo <= 0) return 0;
  return Math.min(100, Math.round((saldo / total) * 100));
}
...
export default function FluxoPorConta() {
  ...
  const contas = (dados?.contas ?? []).map(c => ({ ...c, saldoFinal: c.curva.length ? c.curva[c.curva.length - 1]! : c.saldoInicial }));
  ...
  return (
    <RelatorioShell
      ...
      subtitulo={vazio ? "nenhuma conta bancária cadastrada" : dados ? `${contas.length} ${contas.length === 1 ? "conta acompanhada" : "contas acompanhadas"} · ${dados.periodo.de} a ${dados.periodo.ate}` : "carregando…"}
      vazio={vazio}
      ...
      rodape={!vazio && emQueda.length > 0
        ...
        : "Só lançamentos pagos. Transferências entre contas próprias aparecem nas duas contas envolvidas, mas se compensam no total consolidado."}
    >
              ...
              <article key={c.id} className="flex flex-col gap-[18px] rounded-[20px] bg-white p-6 ring-1 ring-[#E1E8E3]">
                ...
                <div className="flex flex-col gap-2.5 border-t border-[#F1F4F2] pt-4 text-[12.5px]">
                  ...
                  <div className="flex items-center gap-2.5 pt-1">
                    <span className="h-2 flex-1 overflow-hidden rounded bg-[#EDF2EE]"><span className={`block h-full ${c.saldoFinal < c.saldoInicial ? "bg-[#F0A6A0]" : "bg-[#12B85C]"}`} style={{ width: `${peso(c.saldoFinal, total)}%` }} /></span>
                    <span className="whitespace-nowrap text-[12px] text-[#4C6355]">{peso(c.saldoFinal, total)}% do caixa</span>
                  </div>
                </div>
              </article>
            ))}
          ...
          {vazio ? (
            ...
            <section className="rounded-[20px] bg-white p-[26px] ring-1 ring-[#E1E8E3]">
              <div className="flex flex-wrap items-baseline gap-3 pb-3">
                ...
                <span className="text-[12.5px] text-[#4C6355]">transferências entre contas próprias já compensadas no total</span>
              </div>
                ...
                <div className="min-w-[860px]">
                  ...
                  {contas.map(c => (
                    <div key={c.id} className="grid grid-cols-[1.6fr_140px_140px_140px_150px_120px] items-center gap-3 border-b border-[#F1F4F2] px-1 py-3.5">
                      ...
                      <span className="text-right text-[13px] text-[#4C6355]">{peso(c.saldoFinal, total)}%</span>
                    </div>
                  ))}
                  <div className="grid grid-cols-[1.6fr_140px_140px_140px_150px_120px] items-center gap-3 px-1 pb-1 pt-4">
                    <span className="text-[14px] font-bold">Total das contas</span>
                    ...
                    <span className={`text-right text-[16px] font-bold ${total >= 0 ? "text-[#0A7A42]" : "text-[#B3261E]"}`}>{dinheiro(total)}</span>
